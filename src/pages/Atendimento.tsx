...
export default function Atendimento() {
      ...
      if (data.message) {
        ...
        setConversations((prev) => {
          ...
          if (existing) {
              ...
              if (conv.contactPhone === groupKey) {
                return {
                  ...
                  unread:
                    ...
                    selectedPhoneRef.current !== groupKey
                      ? true
                      : conv.unread,
                };
              }
            ...
            const displayName =
              newMsg.isGroup && newMsg.groupName
                ? newMsg.groupName
                : newMsg.contactName;
  ...
  const loadConversations = useCallback(async () => {
    try {
      ...
      allData.forEach((conv) => {
        ...
        const groupKey =
          conv.isGroup && conv.groupId ? conv.groupId : conv.contactPhone;
        ...
        if (existing) {
          ...
          // Para grupos, usar groupName como contactName; para contatos, usar contactName normal
          const displayName =
            conv.isGroup && conv.groupName ? conv.groupName : conv.contactName;
  ...
  useEffect(() => {
    const loadControlPanelSettings = async () => {
      try {
        const settings = await controlPanelService.get(
              ...
              <ScrollArea ref={messagesScrollRef} className="flex-1 p-4">
                <div className="space-y-4" onClick={(e) => e.stopPropagation()}>
                  {selectedConversation.messages.map((msg) => {
                    ...
                    const isMyMessage =
                      msg.sender === "operator" && msg.userId === user?.id;
                    const isOtherOperator =
                      msg.sender === "operator" && msg.userId !== user?.id;
